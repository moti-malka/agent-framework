...
# IMPORTANT: Use --pre flag to install the pre-release version
pip install -r requirements.txt

# Note: requirements.txt includes agent-framework which requires --pre flag
...
# 1. Choose streaming (automatic) or human-in-the-loop (plan review) mode
# 2. Select from 3 pre-defined startup ideas:
#    - AI-Powered Code Review Platform
#    - Sustainable Packaging Marketplace
#    - AI Mental Health Companion for Workplaces
#    Or enter your own custom startup idea
# 3. Watch agents collaborate in real-time
# 4. Review the generated discussion log in discussions/ folder
```
...
| **Discussion Logging** | Custom middleware exports full Markdown conversation logs |
| **Agent Middleware** | Custom logging hooks for observability |
| **Azure Authentication** | Both API key and Azure CLI auth support |
| **Real-time Markdown Export** | Live discussion logs with timestamps and agent personas |
...
├── discussions/               # Auto-created: Markdown logs from magentic_example.py
├── README.md                  # This file
└── README copy.md             # Legacy README version
```
...
The `agent_framework.ipynb` tutorial builds the **InboxOps Support Email Copilot** - a complete learning journey through 12 progressive sections:

| # | Section | What You'll Learn |
|---|---------|------------------|
| **0** | Environment Setup | Azure OpenAI configuration, models, sample data |
| **1** | V0: Basic Agent | Create and run your first agent |
| **2** | Streaming Responses | Real-time token streaming |
| **3** | Multi-Turn Conversations | Thread-based memory and context |
| **4** | V1: Function Tools | Add custom capabilities with `@tool` decorator |
| **5** | V2: Human-in-the-Loop | Approval workflows with `approval_mode` |
| **6** | Middleware & Observability | Logging & monitoring agent behavior |
| **7** | Memory & Context | Persistent user preferences and context |
| **8** | Sequential Workflows | Classify → Draft → Review pipeline |
| **9** | Branching Logic | Conditional routing (Spam vs. NotSpam vs. Uncertain) |
| **10** | Fan-Out/Fan-In | Parallel processing and result aggregation |
| **11** | Multi-Agent Group Chat | Team collaboration patterns |
| **12** | Capstone Demo | End-to-end production-ready system |

## 🔧 Troubleshooting

### Agent Framework Installation Issues

If you see errors like `AttributeError: 'AzureOpenAIChatClient' object has no attribute 'as_agent'`:

```bash
# Uninstall and reinstall with --pre flag
...
**Important:** The `--pre` flag is required to install the pre-release version with all features.
...
If changes don't take effect in Jupyter:
- **Kernel → Restart Kernel** in Jupyter
- Re-run the setup cells
